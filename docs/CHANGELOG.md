...
- 接入可选的 TypeSafe Jev 决策引擎：配置 `jev.api_key` 或 `TYPESAFE_API_KEY` 后，可在上下文压缩前筛除与当前任务无关的旧工具结果和长篇 assistant 消息；判定失败、超时或结果异常时保留原内容。
- 新增 `/permission auto` 模式。未命中用户权限规则的 Shell 命令先由 Jev 判断，明确安全时免审批执行，危险、不确定或服务不可用时继续请求授权；用户 deny、ask、allow 规则和结构化风险检测仍优先或继续生效。
- Web 请求在 auto 模式下增加 URL 风险判定；TUI 展示 Jev 对 Shell 与 URL 的安全、危险、不可用判定，并通过结构化事件传递判定结果，避免工具输出伪造状态提示。
- GUI 配置页新增 Jev 开关、API Key、模型、服务地址和压缩保留阈值；关闭或移除 Jev 后，正在使用的 auto 模式自动回到 manual。

### 远程 MCP 与 OAuth

- MCP 新增 Streamable HTTP 远程服务，支持 HTTPS 地址、授权服务器发现、Authorization Code + PKCE、动态客户端注册、预注册客户端和固定本机回调端口。
- TUI 新增 `/mcp` 状态菜单以及登录、取消和退出登录操作；GUI 可配置远程服务并发起授权。OAuth 凭据按服务、项目和客户端配置隔离保存，支持刷新令牌轮换与失效后重新授权。
- MCP 配置替换改为锁外分阶段连接和原子发布；失败时保留原有可用服务，关闭管理器或取消请求会终止尚未完成的远程连接。
- 加强远程 MCP 与 OAuth 的安全边界：只允许 HTTPS 或回环地址 HTTP，验证重定向和实际连接地址，防止 OAuth 辅助请求访问本地服务，并避免日志、健康状态和错误信息泄露令牌或响应正文。

### 可靠性与体验

- 上下文摘要返回空内容时自动重试一次，并在仍无内容时报告具体错误；Jev 预筛与摘要调用均保留取消、用量统计和失败回退语义。
- 修复 MCP 服务刷新、授权并发、凭据写入与退出登录竞态，以及项目 MCP 命令规范化、配置视图同步等问题。
- 修复 `/mcp` 只读取运行时连接状态、无法识别工作区 MCP 来源的问题；命令与菜单现在合并配置定义和运行状态，并显示已禁用或尚未加载的工作区服务。
- TUI 权限菜单和工具卡片同步展示 auto 模式及可信 Jev 判定；优化系统消息中的授权链接显示与交互。

### 兼容性说明

- 远程 MCP 仅支持 Streamable HTTP；旧式 HTTP+SSE transport 和服务端 URL Elicitation 暂不支持。OAuth 浏览器与 NekoCode 需要运行在同一台机器。
- 本地 MCP 的 `command` 配置保持兼容；远程服务改用互斥的 `url` 字段。Jev 未配置或被禁用时维持原有审批与压缩行为。

## v0.4.5 - 2026-09-19

- 新增项目级工作空间配置：从启动目录读取 `NEKOCODE.md` 作为项目规范并持续提供给主 Agent 和子 Agent，压缩对话不会删除；管理界面与 `/workspace` 可查看来源和读取错误。
- 新增 `<项目>/.nekocode/.mcp.json` 项目 MCP，按名称覆盖同名全局定义（整体替换，不合并 `args`/`env`）；支持 `cwd` 指定子进程工作目录，`enabled: false` 可屏蔽同名全局服务。项目定义不写回用户级配置。
- 同名技能按「项目 → 用户 → 插件 → 内置」优先级选择，取代原先的全局排序；技能与插件目录在实例创建时固定。
- 新增 `/workspace`（查看）与 `/workspace reload`（刷新）命令；管理界面的刷新共用同一流程，在任务之间执行，不监听磁盘变化。
- 项目刷新按「准备 / 发布」两阶段执行：只在锁内取配置副本和上一份完整视图，读文件、扫扩展和处理 MCP 进程都在锁外；发布前管理视图返回旧快照。未变更且正常运行的宿主 MCP 不再重启，处于错误状态的 MCP 会重试。
- 命令注册表改用读写锁，技能命令一次性替换全部条目，刷新期间不再出现半空的命令表；回调在锁外执行，`/help` 等不再有死锁风险。
- 会话恢复前校验 `cwd` 归属，拒绝跨项目恢复；缺少 `cwd` 元数据时明确报错，不再静默绑定当前项目。
- 管理视图将被其他来源覆盖的 MCP 标记为 `shadowed`，不再把实际运行服务的状态和工具数归给被覆盖项；插件快照改为值拷贝，避免视图与 `Enabled` 状态别名。
- MCP 连接提供的同名服务在被覆盖时保留定义，覆盖项删除并刷新后恢复；已撤销的连接不会恢复。
- 项目规范文件不会授予文件访问权限，`cwd` 与 `cd` 不改变项目配置来源；当前仍是单实例绑定单项目，不提供运行中切换项目或同进程多项目执行。
- 调整 Agent 的权限受阻处理：确认某能力被规则挡住、需要更高权限时直接发起最小权限请求，不再先罗列限制或备选方案再等待确认。

### 兼容性说明

- 项目运行数据仍然不进入版本管理：`.gitignore` 继续整体忽略 `.nekocode/`，项目技能、插件、MCP 配置、权限和索引都保留在本地；仓库自身的忽略策略与文档中给使用者的建议相互独立。
- MCP 的 `env` 可能包含密钥。需要共享项目配置结构时，请提交脱敏的 `.mcp.example.json`，再由使用者复制为 `.mcp.json` 并填入本机配置；示例文件不会自动加载。
