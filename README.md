...
| **折叠** | 工具组折叠 · diff 展开 | **多模型** | Anthropic / OpenAI 协议 · 兼容 DeepSeek / MiniMax |
...
| 字段 | 说明 | 必填 |
|------|------|------|
| `provider` | LLM 提供商标识：`deepseek` / `anthropic` / `minimax` 等 | 是 |
| `api_key` | API 密钥 | 是 |
| `model` | 模型名称 | 是 |
| `base_url` | API 端点地址 | 是 |
...
- **多 Provider 网关**：Anthropic 原生协议 + OpenAI 兼容协议统一接入，Prompt Caching / Prefix Cache 优化，Thinking 跨 Provider 互译
