...
<p align="center">
  <img src="docs/images/splash.png" width="45%" alt="启动页">&nbsp;
...
> **前提**：Go 1.25+。GUI 还需要 Node/npm、Wails CLI，Linux 上需要 GTK/WebKitGTK。
...
```bash
go build -o nekocode-tui ./cmd/nekocode-tui
./nekocode-tui
```
...
Nix 用户：`nix-shell shell.nix && source gui/env.sh && wails dev`

> 更多构建选项（Flatpak 等）见下方[构建详解](#构建详解)。
...
| `danger` | ⚠ 警告确认 | `rm` `kill` `git push -f` |
...
cd gui
npm install
npm run build          # 生成 gui/dist/（go:embed 依赖）
cd ..
wails build            # 输出到 build/bin/
```
