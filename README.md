...
| **工具** | 13+ 内置工具（bash/read/write/edit/glob/grep/list/tree/task/todo_write/project_info/web_search/web_fetch/skill/image_gen） | **上下文** | 五级预警水位 · 分层压缩 · 锚点保留 |
| **Hook** | 7 个内置 Hook + 声明式 · 7 种事件点 | | |
...
| **Project Info** | 代码索引（symbol/deps/file/search/skeleton）· Tree-sitter 多语言 · 增量同步 | **文件缓存** | LRU + mtime 去重 · 跨子 Agent 共享 |
...
└──────────────────────────────────────────────────────┘
```
Bot 核心不依赖任何特定 UI 框架。`BotInterface` 定义了完整的 Agent 交互契约——发送消息、流式回调、工具确认、中止控制、模型切换。换个前端只需实现这 11 个方法。
...
- **防幻觉纵深防御**：Hook 引擎（7 内置 Hook）、回环编辑指纹检测、Token 预算管理
...
- **项目感知**：NEKOCODE.md 自动发现 + @include 递归加载，CIndex 代码索引（Tree-sitter 多语言 + 图遍历 + FTS5 搜索），AgentMD 子代理定义
