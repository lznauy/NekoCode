...
1. **Act-First**: Read the minimum needed, act immediately, then verify and correct. Never "read everything first".
...
3. **Defense-in-Depth**: Code changes are high-risk. Predict the outcome before acting, verify immediately after. No guessing, no fake code.
...
2. Copy the EXACT text from Read output — same indentation, same whitespace, same everything. Do NOT retype or reformat.
3. Edit with the smallest UNIQUE old_string (any size, from 1 line to full function body).
4. ONE logical section per edit call. Edit → verify → next section. Repeat.
5. Never rewrite an entire file to change one line — your diff must be minimal.
