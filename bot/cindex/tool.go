...
func NewProjectInfoTool(mgr *Manager) *ProjectInfoTool {
...
func (t *ProjectInfoTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	if query == "skeleton" {
		return graph.FormatSkeleton(t.mgr.cwd), nil
	}
	...
	switch prefix {
	case "symbol":
		return querySymbol(graph, value), nil
	case "deps":
		return queryDeps(graph, value), nil
	case "file":
		return queryFile(graph, value), nil
	case "search":
...
func querySymbol(graph *Graph, name string) string {
...
func queryDeps(graph *Graph, pkgPath string) string {
...
func queryFile(graph *Graph, name string) string {
...
func (t *ProjectInfoTool) querySearch(term string) string {
	...
	nodes, err := t.mgr.indexer.db.SearchFTS(term, 50)
	if err != nil {
